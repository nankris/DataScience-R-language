...
#setwd("C:\\Users\\admin\\Documents\\Jigsaw\\Data exploration and preparation")
### Example: Predicting Medical Expenses ----
## Step 2: Exploring and preparing the data ----
#insurance <- read.csv("insurance.csv",stringsAsFactors = TRUE)

setwd("C:\\Users\\Poori\\Desktop\\jigsaw\\R for data science\\Class Codes")
insurance<-read.csv("C:/Users/Poori/Desktop/jigsaw/R for data science/Class Codes/insurance.csv")
...
#tabulations # good for categorical variables, if you try to do it normal variables then u won't get table
table(insurance$sex)
table(insurance$region)
table(insurance$smoker)

table(insurance$smoker,insurance$region)
...
table(insurance$sex,insurance$smoker,insurance$region)

# table(insurance$smoker,insurance$region, insurance$sex)



#Check for outliers in age column of insurance data
...
index<-which(insurance$age %in% list) # here list is values in which x$out are present as assigned above

# this is another way of doing above code
index<-which(insurance$age==x$out)
...
# replace NA in bmi column with mean of bmi column
insurance$bmi[is.na(insurance$bmi)]<-mean(insurance$bmi,na.rm=TRUE)
#dim(insurance)
sum(is.na(insurance$bmi))

insurance$bmi[3]=NA
sum(is.na(insurance$bmi))

insurance$bmi[is.na(insurance$bmi)]=mean(insurance$bmi, na.rm=FALSE) #changes won't reflect because
# we are not removing NA from the insurance data set so it won't compute mean
print(mean(insurance$bmi,na.rm=FALSE)) #we are replacing NA by NA only so we won't get desired result

sum(is.na(insurance$bmi))

# sum(is.na(insurance$bmi)) # this should give you 0 output once you run above code
...
aggregate(charges ~ smoker+sex , data=insurance, FUN=mean) #What inference?
...
aggregate(charges ~ region + smoker , data=insurance, FUN=mean)#What inference?
...
ggplot(insurance,aes(x=charges)) + 
  geom_histogram(aes(fill=as.factor(smoker)),position="dodge") + facet_grid(region~.)


ggplot(insurance,aes(x=charges)) + geom_histogram(aes(fill=as.factor(smoker)),position="dodge") + facet_grid(sex~.)
ggplot(insurance,aes(x=charges)) + geom_histogram(aes(fill=as.factor(smoker)),position="dodge") + facet_grid(sex~region)

#box plots : 
ggplot(insurance,aes(y=charges,x=sex,fill=as.factor(sex)))+ geom_boxplot()

ggplot(insurance,aes(y=charges,x=region,fill=as.factor(region)))+ geom_boxplot()+ facet_grid(smoker~.)
...
insurance$dummy_r_se<-ifelse(insurance$region=="southeast",1,0) #Using ifelse

# insurance$dummy_r_ne
# length(insurance$dummy_r_ne)
...
insurance_new<-cbind(insurance,y) # cbind adds columns
...
# below code is from Visualization.R
...
# 27 april q and a class 

# hypothesis is an assumption which is yet to be proved
'but take for the argument
we have to look at null hypothesis and alternate
hypothesis

Discrete distributions: Binomial and poisson
Continuous distributions: Normal distribution and t distribution

When to apply binomial
When the outcome of the experiment is two and
the variable must be discrete.

Wheather the customer is defaulter or not etc.
we need to have n and p parameters to apply
binomial distribution. n no. of trails, p probability
of distribution

poisson: it is time bound. An event which is occuring
on a cettain time period

Normal N>= 30 population mean and population standard deviation/ sample standard

.R, .Rdata/.rda, .RHistory are created when we
work with Rstudio
...
prepare the data
start your model building process by doing multiple iterations

# books: statistics for management and economy by gerald keller


we have to test assumptions on normality (condition to normality)
error should be normally distributed
heteroscetasticity we have to look for

1) normality of error
2) hetroscedasticity

matrix to dataframe we use as.data.frame, we convert a object to data framae

data.frame will create a data frame

Book: introduction to statistical learning with application in R

Goodness of fit for the linear regression:
First we have to look at R2 and then the line chart
we can look at  MAE or MSE or RMSE 
MAE mean absolute error
MSE mean square error
RMSE root mean square error

cat function output gives NULL
paste0 function output gives character output

when we are working with folder parts then paste0 is more useful

list.files() also used

when to use negative binomial distribution:
in binomila distribution we are trying to predict
x success in n trails for given p

in negative binomial distribution you will
find probability for xth success in nth trail for
given p

Ex: probability of RCB winning 5th game in 
12th match

# 4th may class. time series
exogeneous
endogeneous
exogeneous is variable that isnot affected by
other variables in the system. Used for causality

Endogeneous are affected by other variables.Ex: price
  
seasonality is calculated from target variable itself

Trend, seasonality, cyclicity and irregularity all are calculated
from the target variable itself

irregular component is: we cant answer from the trend
then we say noise in the data that you cannot answer

Seasonality: short term movements that happens
and takes place. If there is peak there may be dip also

Dip can also have seasonal affect

In some months sales are having dip then this 
is due to seasonality. 

Dip of sales of AC in winter has dip

Trend is more year on year affect
seasonality is one time short term affect
seasonality can be annyally, monthly, quartarly

If happening every month then trend this is what
differentiates seasonality from trend

Cyclicity: A professional is changing his/her
job every 5 years. Then the month he/she joins the
new company can we see certain affect? We see signing bonus
relocation charges etc. Every 5 years we see kind of
cycle get repeated

cyclicity: long term oscillations that happen
in time.

Every 5 years, elections are there then share
markets are having cyclicity happens

Any fluctuations that cant be explained using trend
seasonality and cyclicity is irregularity


# STL seasonality, trend, L stands for cyclicity

decompose function and STL function we used
to seperate

# after breaking time series into different components
then we will forecast

Forecasting
Exponential Smoothening (another one is lowess smoothening)



Use this code to install qualprops library

in r (not in r studio) try below commands
install.packages("installr") # install installr
library(installr) #load

Select anything from the US (CRAN mirror)

Once done you can install install.packages("quadprog")
in R studio

We are using R 3.5.3
This is the step to upgrade


Upgrade r from 3.5.3 to 3.6
